...
# 1) Vypsání aktuálního datumu a času
now = datetime.datetime.now()
print("Aktuální datum a čas:", now)

# 2) Vypsání datumu velikonoční neděle v následujících 5 letech
for i in range(5):
    easter = holidays.easter.easter(year=now.year + i)
    print(f"Velikonoční neděle v roce {now.year + i}: {easter}")

# 3) Vypsání nejbližšího roku, kdy bude Štědrý den v neděli
current_year = now.year
while True:
    christmas = datetime.date(current_year, 12, 25)
    if christmas.weekday() == 6:  # Neděle má index 6
        print(f"Štědrý den bude v neděli v roce {current_year}")
        break
    current_year += 1
